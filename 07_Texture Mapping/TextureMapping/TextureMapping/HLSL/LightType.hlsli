...
//输入材质，方向光，着色点表面的法线，看向眼睛方向，输出着色点发出的环境光，漫反射光和镜面反射光
{
    ...
    {
        float3 ReflectVec = 2 * DiffuseFactor * normal - LightVec; //reflect(-LightVec, normal); 反射光:2*DiffuseFactor*normal-LightVec
...
//输入材质，方向光，着色点位置，表面的法线，看向眼睛方向，输出着色点发出的环境光，漫反射光和镜面反射光
{
    ...
    if (DiffuseFactor > 0.0f)
    {
        float3 ReflectVec = 2 * DiffuseFactor * normal - LightVec; //反射光:2*DiffuseFactor*normal-LightVec
        float SpecularFactor = pow(max(dot(ReflectVec, toViewPos), 0), mat.Specular.w);
